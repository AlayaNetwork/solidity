...
FunctionValue TestFileParser::parseFunctionCallValue()
{
	try
	{
		...
		if (token != Token::Atp && token != Token::Von)
			BOOST_THROW_EXCEPTION(TestParserError("Invalid value unit provided. Coins can be von or atp."));
	...
	{
		BOOST_THROW_EXCEPTION(TestParserError("Atp value encoding invalid."));
	}
}
